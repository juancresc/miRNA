...
#data_lib <- "files"
#data_lib <- '21dpi_iwgsc'
#countdata <- read.table(paste("data/", data_lib, "/",file,'.csv',sep=""), header=TRUE)
#Run once with each file name
#file = 'valid_cluster'
...
# Load data
countdata <- read.table("data/files/valid_counts.csv",header=TRUE)
#countdata <- read.table("data/files/counts.clean.txt",header=TRUE)
result_path <- "data/files/"
